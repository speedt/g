...
for i=2, #group_pos, 2 do
  ...
  if (dsb) then
    ...
    table.insert(user_info, redis.call('HGET', 'prop::user::'.. u, 'vip'));
    table.insert(user_info, redis.call('HGET', 'prop::user::'.. u, 'score'));
    table.insert(user_info, redis.call('HGET', 'prop::user::'.. u, 'gift_count'));
  else
